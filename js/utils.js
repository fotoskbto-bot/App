...
export function checkConsecutiveAbsences(attendanceRecords, userId, daysThreshold = 3) {
    ...
    if (userAttendance.length < daysThreshold) return false;
    
    // Verificar si las últimas inasistencias son consecutivas
    for (let i = 0; i < daysThreshold - 1; i++) {
        const currentDate = new Date(userAttendance[i].date);
        const nextDate = new Date(userAttendance[i + 1].date);
        const diffDays = Math.abs((currentDate - nextDate) / (1000 * 60 * 60 * 24));
        
        if (diffDays > 1) return false; // No son consecutivas
    }
